package com.pdf.reader.services.util;
...
import com.pdf.reader.util.ProjectConfig;
...
//		logger.info("Input Parameters, folder=" + folder);
		...
		String filePath = ROOT_DIRECTORY + File.separator + SERVER_NAME  + File.separator+ filecount + getExtension(fileName);
		...
		if (config.getSecureProtocolContext()){
			 image_url=config.getSecureImagePath()+  File.separator+ filecount + getExtension(fileName);
			 
			return config.getSecureImagePath()+  File.separator+ filecount + getExtension(fileName);
		}
		else
			return config.getUnsecureImagePath()+  File.separator+ filecount + getExtension(fileName);
